...
def get_weather(lat, lon):
    """
    Fetch current weather conditions for the given latitude and longitude.
    Returns a dict with Temperature, Humidity, and Conditions, or None values on failure.
    """
    if lat is None or lon is None:
        return {"Temperature": None, "Humidity": None, "Conditions": None}

    # Build Visual Crossing Timeline endpoint URL
    vc_url = (
    ...
    try:
        ...
        # Log or display a warning in the app
        st.warning(f"Weather fetch failed for {lat},{lon}: {e}")
        return {"Temperature": None, "Humidity": None, "Conditions": None}
